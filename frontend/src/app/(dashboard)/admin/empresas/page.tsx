...
import type { AccountingFirm, CnpjLookupResult, Company } from "@/types";
...
function companyToFormValues(c: Company): CompanyForm {
  return {
    ...
    cnpj: c.cnpj,
    uf: c.uf,
    accounting_firm_id: c.accounting_firm_id,
...
export default function AdminEmpresasPage() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([api.get("/admin/companies"), api.get("/admin/accounting-firms")])
      .then(([companiesRes, firmsRes]) => {
  ...
  const onCreate = async (data: CompanyForm) => {
    ...
    try {
      ...
      setCreateOpen(false);
      createForm.reset({ uf: "PA", accounting_firm_id: "" });
  ...
  const toggleActive = async (company: Company) => {
    try {
      await api.patch(`/admin/companies/${company.id}`, { is_active: !company.is_active });
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Empresas clientes de todos os escritórios</p>
        </div>
        <button
              ...
              {companies.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-gray-600">
                    {firmNameById.get(c.accounting_firm_id) ?? "—"}
                  </td>
                  <td className="px-6 py-4 text-gray-600">
      ...
      >
        <form onSubmit={createForm.handleSubmit(onCreate)} className="space-y-5">
          <Field label="Escritório" error={createForm.formState.errors.accounting_firm_id?.message}>
            <select
      ...
      >
        <form onSubmit={editForm.handleSubmit(onEdit)} className="space-y-5">
          <div>
            ...
            <input
              disabled
              value={editing ? firmNameById.get(editing.accounting_firm_id) ?? "—" : ""}
