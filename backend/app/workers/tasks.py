...
def _get_sessionmaker() -> async_sessionmaker:
    """Sessionmaker padrão da task, criado sob demanda. Injetável em testes
    via o parâmetro `session_factory` de run_sped_processing, para que o
    teste e a chamada à task compartilhem o mesmo engine/banco."""
...
async def run_sped_processing(
    ...
    SessionLocal = session_factory or _get_sessionmaker()
        ...
        try:
            ...
            if unmatched:
                for ant in unmatched:
                    db.add(JobLog(
                        ...
                        message=(
                            f"Sem correspondência no SPED: NF {ant.numero_nf} série {ant.serie} "
                            f"CNPJ {ant.emitente_cnpj} tipo {ant.tipo}"
                        ),
                    ))
            await db.commit()
            ...
            db.add(JobLog(
                ...
                message=(
                    ...
                    f"{sped_result.e111_inserted} E111, {sped_result.e116_inserted} E116 inseridos"
                ),
            ))
            await db.commit()
